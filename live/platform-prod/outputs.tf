# Outputs consumed by `data-prod`, `cluster-prod` and every product stack.
#
# THIS IS THE CONTRACT THAT REPLACES `runtime-prod`'s. The names are deliberately
# IDENTICAL to the ones runtime-prod exports, so repointing a consumer is a
...
output "vpc_id" {
  ...
  description = "VPC ID — consumed by cluster-prod for its S3 gateway endpoint."
}
...
output "cluster_subnet_ids" {
  ...
  description = <<-EOT
    The /20 tier EKS nodes and pods live in. `cluster-prod` reads this.
...
output "data_subnet_ids" {
  ...
  description = "Data subnet IDs — the shared Postgres and cache in data-prod."
}
